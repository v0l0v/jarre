    ...
    <script src="planets.js">
</head>
<body>
    <div id="tv-noise"></div>
    <div id="game-map">
        <button id="close-map" onclick="closeMap()">X</button>
        <div id="map-header">
            <input type="text" id="map-search" placeholder="Buscar planeta..." autocomplete="off">
    ...
    <a href="#" onclick="toggleFullScreen(document.documentElement); return false;" class="fullscreen-btn" title="Pantalla Completa">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
    ...
    <button id="btn-map" class="hud-btn" onclick="openMap()" title="Abrir Mapa Estelar">
    ...
    <button id="btn-term" class="hud-btn" onclick="openTerminal()" title="Abrir Terminal de BASH">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
    ...
    <div id="planet-info">
        ...
        <p id="info-desc">Haz clic para iniciar el salto hiperespacial</p>
    </div>
    ...
    <div class="hint">Mueve el ratón para explorar el espacio</div>
        ...
        function triggerDirectionalJump(direction) {
            ...
            for (let i = 0; i < worldPalettes.length; i++) {
                ...
                if (worldPalettes[i].secret) continue; // Excluir Sector Omega de los saltos convencionales
        ...
        function changeWorld(idx, skipWarp = false) {
            if (currentWorld === SECRET_INDEX && idx !== SECRET_INDEX) {
                omegaDiscovered = false;
            }
            currentWorld = idx;
            alternativePortals.forEach(p => p.mesh.visible = false);
            document.querySelectorAll('.planet-term').forEach(el => el.style.display = 'none');
        ...
        // --- OMEGA WARP ENGINE ---
        const omegaWarpGroup = new THREE.Group();
        scene.add(omegaWarpGroup);
        const warpGeoPlane = new THREE.PlaneGeometry(3000, 3000);
        const warpGeoSphere = new THREE.SphereGeometry(600, 32, 32);
        const warpGeoCylinder = new THREE.CylinderGeometry(2, 2, 800, 8);
        const warpColors = [0xff00ff, 0x00ffff, 0xffaa00, 0xff0000, 0x00ff00, 0x0000ff, 0xffffff];
        
        const tl = new THREE.TextureLoader();
        const nebulaTex = tl.load('assets/warp/nebula.jpg');
        const bhTex = tl.load('assets/warp/blackhole.jpg');
        
        let warpActiveMeshes = [];

        function spawnWarpObject() {
            let mesh;
            let type = Math.random();
            if (type < 0.3) {
                // Nebulosa
                const mat = new THREE.MeshBasicMaterial({ map: nebulaTex, transparent: true, opacity: 0.8, blending: THREE.AdditiveBlending, depthWrite: false });
                mesh = new THREE.Mesh(warpGeoPlane, mat);
                mesh.userData = { speed: 8000, type: 'nebula', rot: (Math.random() - 0.5) * 0.05 };
            } else if (type < 0.6) {
                // Agujero negro
                const mat = new THREE.MeshBasicMaterial({ map: bhTex, transparent: true, opacity: 0.9 });
                mesh = new THREE.Mesh(warpGeoPlane, mat);
                mesh.userData = { speed: 6000, type: 'blackhole', rot: (Math.random() - 0.5) * 0.02 };
            } else {
                // Estrellas fugaces puras
                const mat = new THREE.MeshBasicMaterial({ color: 0xffffff, transparent: true, opacity: 0.9 });
                mesh = new THREE.Mesh(warpGeoCylinder, mat);
                mesh.rotation.x = Math.PI / 2;
                mesh.userData = { speed: 25000, type: 'star' };
            }
            
            const spreadX = (Math.random() - 0.5) * 6000;
            const spreadY = (Math.random() - 0.5) * 6000;
            mesh.position.set(camera.position.x + spreadX, camera.position.y + spreadY, camera.position.z - 12000);
            
            // Centrar ligeramente nebulosas y agujeros
            if (mesh.userData.type !== 'star') {
                mesh.position.x = camera.position.x + (Math.random() - 0.5) * 1500;
                mesh.position.y = camera.position.y + (Math.random() - 0.5) * 1500;
            }
            omegaWarpGroup.add(mesh);
            warpActiveMeshes.push(mesh);
        }

        function clearWarpEngine() {
            for (let i = warpActiveMeshes.length - 1; i >= 0; i--) {
                omegaWarpGroup.remove(warpActiveMeshes[i]);
            }
            warpActiveMeshes = [];
        }
        ...
        function render() {
            ...
            if (isWarpSpeed) {
                ...
                if (pendingWorldIndex === SECRET_INDEX) {
                    const cosmicFlash = document.getElementById('cosmic-flash');
                    if (warpTimer <= 2.0 && warpTimer > 0) {
                        cosmicFlash.style.opacity = '1';
                    }
                    
                    // Omega Warp Visuals
                    if (Math.random() < 0.6) spawnWarpObject();
                    for (let i = warpActiveMeshes.length - 1; i >= 0; i--) {
                        const mesh = warpActiveMeshes[i];
                        mesh.position.z += mesh.userData.speed * delta;
                        if (mesh.userData.type !== 'star') mesh.rotation.z += mesh.userData.rot;
                        if (mesh.position.z > camera.position.z + 1000) {
                            omegaWarpGroup.remove(mesh);
                            warpActiveMeshes.splice(i, 1);
                        }
                    }
                }
            } else {
                if (warpActiveMeshes.length > 0) clearWarpEngine();
            }lice(i, 1);
                        }
                    }
                }
                
                // Movimiento hacia el objetivo del salto
                ...
                if (warpTimer <= 0) {
                    ...
                    document.getElementById('cosmic-flash').style.opacity = '0';
